...
export default async function ModerationCasePage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{saved?:string}>}){
  ...
  return <>
    ...
    <div className={styles.detailGrid}><div>
      ...
      <section className={styles.section}><h2>Case details</h2><div className={styles.meta}><div><span>Confidence</span><strong>{pct(c.confidence)}</strong></div><div><span>Would recommend</span><strong>{c.recommended_action.replaceAll('_',' ')}</strong></div><div><span>Detected</span><strong>{when(c.created_at)}</strong></div><div><span>Channel</span><strong>{c.channel_name?`#${c.channel_name}`:c.channel_id}</strong></div><div><span>Offense level</span><strong>#{c.offense_number||1}</strong></div><div><span>Prior confirmed</span><strong>{c.prior_confirmed_count}</strong></div><div><span>Repeat window</span><strong>{c.repeat_window_days_used?`${c.repeat_window_days_used} days`:'—'}</strong></div><div><span>Mode</span><strong>Observe only</strong></div></div></section>
