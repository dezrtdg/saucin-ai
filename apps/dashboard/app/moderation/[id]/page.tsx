...
export default async function ModerationCasePage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{saved?:string}>}){
  ...
  try{feedback=(await api<FeedbackData>(`/api/moderation/cases/${id}/feedback`)).feedback||[];}catch{}
  ...
  const playerContested=Boolean(c.player_contested_at);
  ...
  return <>
    ...
    {playerContested?<div className={styles.info}><strong>Player marked this detection Incorrect:</strong> review the saved conversation context and second-pass context analysis below. The player click does not automatically overturn the action or teach the classifier by itself.</div>:null}
    ...
    <div className={styles.detailGrid}><div>
      ...
      <section className={styles.section}><h2>Conversation context</h2><p>This is the preceding conversation window saved with the moderation case. It is the context used to distinguish direct violations from quoting, reporting, banter, or other context-sensitive situations.</p><p className={styles.evidence}>{c.context_snapshot||'No conversation context was captured for this case. Older cases created before context capture may not have a snapshot.'}</p></section>
      ...
      {feedback.length?<section className={styles.section}><h2>Player feedback</h2><p>Player feedback is kept as evidence. Only a human staff Confirm/Dismiss outcome becomes trusted calibration data for future moderation tuning.</p>{feedback.map(f=>{const r=f.reanalysis||{};return <div className={styles.escalationBox} key={f.id}><strong>{f.feedback_type==='incorrect'?'Incorrect / contested':'I understand / acknowledged'} · {when(f.created_at)}</strong>{f.feedback_type==='incorrect'?<><span>{r.available?(r.matched?`Second-pass result: still matches at ${pct(r.confidence||0)} confidence.`:`Second-pass result: no match at ${pct(r.confidence||0)} confidence.`):'Second-pass context analysis was unavailable.'}</span>{r.reason?<span>{r.reason}</span>:null}{r.context_factors?.length?<span>Context factors: {r.context_factors.join(' · ')}</span>:null}</>:<span>The member acknowledged receiving and understanding the notice.</span>}<span>{f.trusted_staff_outcome?`Trusted staff outcome recorded: ${f.case_status}.`:'Awaiting a human staff outcome before this feedback can become trusted calibration data.'}</span></div>})}</section>:null}
      ...
      {can(access,'moderation.review')?<section className={`${styles.section} ${styles.reviewBox}`}><h2>{playerContested?'Player-contested review / correction':staffFollowup?'Staff follow-up / correction':'Staff review / correction'}</h2><form action={reviewModerationCaseAction.bind(null,id,'confirmed')}><textarea className="textarea" name="notes" defaultValue={c.review_notes||''} placeholder="Optional staff notes…"/><div className={styles.reviewActions}>{c.status!=='confirmed'||playerContested?<ActionButton label="Confirm detection" className={styles.confirm}/>:null}<ActionButton label="Dismiss / remove from escalation" className={styles.dismiss} formAction={reviewModerationCaseAction.bind(null,id,'dismissed')}/>{c.status==='dismissed'?<ActionButton label="Reopen" formAction={reviewModerationCaseAction.bind(null,id,'pending')}/>:null}</div></form></section>:null}
