...
export default async function ModerationCasePage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{saved?:string}>}){
  ...
  const staffReviewRequired=Boolean(c.staff_review_required||(c.offense_number||1)>=4);
  ...
  return <>
    ...
    <div className={styles.detailGrid}><div>
      ...
      <section className={styles.section}><h2>AI assessment</h2><p>{c.ai_reason||'No additional rationale was saved.'}</p>{c.evidence?<><h2>Evidence</h2><p className={styles.evidence}>{c.evidence}</p></>:null}<div className={styles.info}>{isLiveCase?'This detection was eligible for the standard live action shown below. Staff confirmation still controls whether this case can advance a future repeat offense.':'Observe Mode does not warn, delete, or timeout the member.'}</div><div className={styles.escalationBox}><strong>Repeat-offense calculation</strong><span>This was evaluated as offense #{c.offense_number||1} because Saucin AI found {c.prior_confirmed_count||0} prior staff-confirmed case{Number(c.prior_confirmed_count||0)===1?'':'s'} for this same rule within {c.repeat_window_days_used||'the configured'} day{Number(c.repeat_window_days_used||0)===1?'':'s'}. Pending or dismissed cases do not advance the ladder.</span></div></section>
