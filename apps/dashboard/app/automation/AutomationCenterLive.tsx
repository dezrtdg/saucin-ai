...
type LearningStats={active:boolean;totals:{trusted:number;helpful_30d:number;corrections_30d:number};modules:Record<string,{trusted:number;helpful_30d:number;corrections_30d:number}>};
export type AutomationData={generated_at:string;modules:Module[];items:Item[];counts:Record<ModuleKey,number>;total:number;learning:LearningStats};
...
function dataSignature(data:AutomationData){return JSON.stringify({modules:data.modules.map(item=>[item.key,item.autonomy_level,item.updated_at]),items:data.items.map(item=>[item.key,item.status,item.priority_score,item.review_outcome,item.reviewed_at]),counts:data.counts,total:data.total,learning:data.learning});}
...
export default function AutomationCenterLive({initialData,access,moduleFilter,includeReviewed}:{
  ...
  return <div ref={rootRef}>
    ...
    <section className={styles.learningPanel}>
      <div><span className={styles.learningPulse}/><div><small>LEARNING ENGINE</small><strong>Staff-calibrated and active</strong></div></div>
      <dl><div><dt>{data.learning.totals.trusted}</dt><dd>trusted examples</dd></div><div><dt>{data.learning.totals.helpful_30d}</dt><dd>confirmed this month</dd></div><div><dt>{data.learning.totals.corrections_30d}</dt><dd>corrections this month</dd></div></dl>
      <p>Only staff-reviewed outcomes teach future decisions. Player claims never become verified facts or punishments.</p>
    </section>
    ...
    <section className={`issueViewPanel ${styles.queue}`}><div className="issueSectionHeader"><div><span className="sectionLabel">UNIFIED REVIEW INBOX</span><h2>{moduleFilter?moduleCopy[moduleFilter]:'All work requiring attention'}</h2><p>Priority is based on impact, confirmations, recurrence, and whether a human decision is required.</p></div><span className="countPill">{items.length}</span></div><nav className={styles.filters}><Link className={!moduleFilter?styles.active:''} href={includeReviewed?'/automation?reviewed=1':'/automation'}>All</Link>{data.modules.map(module=><Link className={moduleFilter===module.key?styles.active:''} key={module.key} href={`/automation?module=${module.key}${includeReviewed?'&reviewed=1':''}`}>{module.label}<b>{data.counts[module.key]||0}</b></Link>)}</nav>
      <div className={styles.items}>{items.length?items.map(item=><article className={styles.item} key={item.key}><div className={styles.itemMain}><div className={styles.itemMeta}><span className={`${styles.priority} ${styles[item.priority]}`}>{item.priority}</span><span>{moduleCopy[item.module]}</span><span>{item.status.replaceAll('_',' ')}</span><time>{date(item.created_at)}</time></div><h3><Link href={item.href}>{item.title}</Link></h3>{item.detail?<p>{item.detail}</p>:null}<small>{item.reason}</small>{item.review_outcome?<em>Reviewed as {item.review_outcome}{item.reviewed_at?` · ${date(item.reviewed_at)}`:''}</em>:null}</div><div className={styles.itemActions}><Link className="button primary" href={item.href}>Open</Link>{can(access,'automation.review')?(item.review_outcome?<form action={automationFeedbackAction.bind(null,item.module,item.resource_type,item.resource_id,'reopened')}><ActionButton label="Return to inbox" pendingLabel="Returning" variant="subtle"/></form>:item.resource_type==='ticket_followup'?<form action={automationFeedbackAction.bind(null,item.module,item.resource_type,item.resource_id,'reviewed')}><ActionButton label="Acknowledge" pendingLabel="Saving" variant="subtle"/></form>:<><form action={automationFeedbackAction.bind(null,item.module,item.resource_type,item.resource_id,'helpful')}><ActionButton label="Looks right" pendingLabel="Learning" variant="subtle"/></form><details className={styles.correction}><summary>Correct this decision</summary><form action={automationFeedbackAction.bind(null,item.module,item.resource_type,item.resource_id,'incorrect')}><label>Correct route<select className="input select" name="corrected_module" defaultValue=""><option value="">Keep current route</option>{data.modules.filter(module=>module.key!==item.module).map(module=><option value={module.key} key={module.key}>{module.label}</option>)}</select></label><label>Correct priority<select className="input select" name="corrected_priority" defaultValue=""><option value="">Keep current priority</option>{(['urgent','high','normal','low'] as const).filter(priority=>priority!==item.priority).map(priority=><option value={priority} key={priority}>{priority}</option>)}</select></label><label className={styles.correctionNote}>Staff note<input className="input" name="note" required placeholder="What should Saucin AI learn?"/></label><ActionButton label="Save correction" pendingLabel="Learning" variant="subtle"/></form></details></>):null}</div></article>):<div className="emptyLibrary"><strong>You’re all caught up</strong><span>No items currently need review in this view.</span></div>}</div>
    </section>
  </div>;
}
