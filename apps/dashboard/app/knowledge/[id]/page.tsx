...
export default async function KnowledgeDetailPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{from?:string}>}){
  ...
  return <>
    ...
    <header className="articleViewHeader">
      ...
      {(can(access,'knowledge.ai')||can(access,'knowledge.edit'))?<div className="headerActions">{can(access,'knowledge.ai')&&can(access,'knowledge.edit')?<form action={improveKnowledgeAction}><input type="hidden" name="id" value={article.id}/><ActionButton label="✨ Improve with AI" pendingLabel="Improving…"/></form>:null}{can(access,'knowledge.edit')?<Link className="button primary" href={`/knowledge/${article.id}/edit?from=${encodeURIComponent(returnScope)}`}>Edit</Link>:null}</div>:null}
    </header>
    ...
    <div className="articleAuditBar"><div><span>Created</span><strong>{prettyDate(article.created_at)}</strong></div><div><span>Created by</span><strong>{article.created_by||'Unknown'}</strong></div><div><span>Embedding</span><strong>{article.embedding_updated_at?'Indexed':'Pending'}</strong></div>{(can(access,'knowledge.publish')||can(access,'knowledge.archive'))?<div className="articleStatusActions">{article.status!=='published'&&can(access,'knowledge.publish')?<form action={setKnowledgeStatusAction}><input type="hidden" name="id" value={article.id}/><input type="hidden" name="return_scope" value={returnScope}/><input type="hidden" name="status" value="published"/><ActionButton label="Publish" variant="primary"/></form>:null}{article.status!=='archived'&&can(access,'knowledge.archive')?<form action={setKnowledgeStatusAction}><input type="hidden" name="id" value={article.id}/><input type="hidden" name="return_scope" value={returnScope}/><input type="hidden" name="status" value="archived"/><ActionButton label="Archive"/></form>:article.status==='archived'&&can(access,'knowledge.archive')?<form action={setKnowledgeStatusAction}><input type="hidden" name="id" value={article.id}/><input type="hidden" name="return_scope" value={returnScope}/><input type="hidden" name="status" value="draft"/><ActionButton label="Restore to draft"/></form>:null}</div>:null}</div>
  </>;
}
