...
export async function POST(request:Request,context:{params:Promise<{id:string}>}){
  ...
  try{
    ...
    const result=await api(`/api/suggestions/${id}/reply/ai-draft`,{
      method:'POST',
    ...
    return NextResponse.json({error:error instanceof Error?error.message:'Unable to draft suggestion update.'},{status:500});
  }
}
