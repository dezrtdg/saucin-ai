...
export default async function SettingsPage(){
  ...
  const sections=[
    ...
    {href:'/settings/moderation',permission:'moderation.configure',eyebrow:'MODERATION',title:'Observe mode',description:'Configure rule detection, confidence thresholds, exemptions, audit logging, and future recommended actions.',status:'Configure'},
  ...
  return <>
    ...
    <div className="settingsHubGrid">{sections.map(section=><Link className="settingsHubCard" href={section.href} key={section.href}><div><span className="sectionLabel">{section.eyebrow}</span><h2>{section.title}</h2><p>{section.description}</p></div><span className="settingsHubAction">{section.status} →</span></Link>)}</div>
    <section className="settingsComingSoon"><div><span className="sectionLabel">COMING NEXT</span><h2>More controls will use the same permission system</h2><p>Announcements, suggestions, integrations, and txAdmin permissions are already reserved so those modules can plug into the same role matrix as they are built.</p></div></section>
  </>;
}
