...
type Audience = { key: string; label: string; description: string; public_access: boolean; sort_order: number; enabled: boolean; role_ids: string[] };
...
export default async function KnowledgeSettingsPage() {
  ...
  let loadError = '';
  try { settings = await api<Settings>('/api/knowledge/settings'); }
  catch (error) { loadError = error instanceof Error ? error.message : 'Unable to load knowledge settings.'; }

  return <>
    <header className="pageHeader">
      <div><p className="eyebrow">SETTINGS</p><h1>Knowledge Settings</h1><p>Control content types, categories, audiences, and which Discord roles unlock restricted knowledge.</p></div>
    ...
    <section className="panel settingsSection">
      <div className="panelTitle"><div><h2>Content types</h2><p>Separate server rules, Discord rules, guides, SOPs, and other knowledge. Discord Rules can be marked as moderation-eligible.</p></div></div>
      <DirectSettingsForm operation="knowledge.contentType.create" className="settingsCreateRow audienceCreateRow" layout="inline" idleLabel="Add content type" pendingLabel="Adding content type…" successMessage="Content type created." buttonClassName="button primary" refreshOnSuccess resetOnSuccess>
      ...
      <div className="settingsRows">
        {settings.content_types.map(type => <div className="settingsRow" key={type.key}>
          <DirectSettingsForm operation="knowledge.contentType.update" resourceId={type.key} className="settingsEditForm" layout="inline" idleLabel="Save" pendingLabel="Saving…" successMessage="Content type saved." buttonClassName="button" refreshOnSuccess>
          ...
          <DirectSettingsForm operation="knowledge.contentType.delete" resourceId={type.key} className="inlineDelete" layout="button" idleLabel="Delete" pendingLabel="Deleting…" successMessage="Content type deleted." buttonClassName="button danger subtle" refreshOnSuccess confirmMessage={`Delete the ${type.label} content type? Existing articles will block deletion.`}/>
        </div>)}
      </div>
    </section>
    ...
    <section className="panel settingsSection">
      ...
      <DirectSettingsForm operation="knowledge.category.create" className="settingsCreateRow" layout="inline" idleLabel="Add category" pendingLabel="Adding category…" successMessage="Knowledge category created." buttonClassName="button primary" refreshOnSuccess resetOnSuccess>
      ...
      <div className="settingsRows">
        {settings.categories.map(category => <div className="settingsRow" key={category.key}>
          <DirectSettingsForm operation="knowledge.category.update" resourceId={category.key} className="settingsEditForm" layout="inline" idleLabel="Save" pendingLabel="Saving…" successMessage="Knowledge category saved." buttonClassName="button" refreshOnSuccess>
          ...
          <DirectSettingsForm operation="knowledge.category.delete" resourceId={category.key} className="inlineDelete" layout="button" idleLabel="Delete" pendingLabel="Deleting…" successMessage="Knowledge category deleted." buttonClassName="button danger subtle" refreshOnSuccess confirmMessage={`Delete the ${category.label} knowledge category? Existing articles will block deletion.`}/>
        </div>)}
      </div>
    </section>

    <section className="panel settingsSection">
      <div className="panelTitle"><div><h2>Audiences & Discord roles</h2><p>A member can use an audience if it is public or any of their Discord roles are mapped to it. Map the same role to multiple audiences to create inheritance.</p></div></div>
      <div className="audienceExplanation"><strong>Example:</strong> To make Community Admins inherit Staff knowledge, select the Community Admin role under both <em>Staff</em> and <em>Admin</em>.</div>

      <DirectSettingsForm operation="knowledge.audience.create" className="settingsCreateRow audienceCreateRow" layout="inline" idleLabel="Add audience" pendingLabel="Adding audience…" successMessage="Knowledge audience created." buttonClassName="button primary" refreshOnSuccess resetOnSuccess>
      ...
      <div className="audienceSettingsList">
        {settings.audiences.map(audience => <div className="audienceSettingsCard" key={audience.key}>
          <DirectSettingsForm operation="knowledge.audience.update" resourceId={audience.key} idleLabel="Save audience & role mapping" pendingLabel="Saving audience…" successMessage="Audience and role mapping saved." actionsClassName="audienceCardActions" refreshOnSuccess>
            ...
            <fieldset className="roleMapping">
              ...
              {settings.discord_roles.length ? <div className="roleGrid">{settings.discord_roles.map(role => <label className="roleChoice" key={role.id}><input type="checkbox" name="role_ids" value={role.id} defaultChecked={audience.role_ids.includes(role.id)} /><span><strong>{role.name}</strong><small>{role.id}</small></span></label>)}</div> : <div className="empty smallEmpty">Discord roles could not be loaded. Make sure the bot is online, then refresh.</div>}
            </fieldset>
          </DirectSettingsForm>
          <DirectSettingsForm operation="knowledge.audience.delete" resourceId={audience.key} className="audienceDelete" layout="button" idleLabel="Delete audience" pendingLabel="Deleting audience…" successMessage="Knowledge audience deleted." buttonClassName="button danger subtle" refreshOnSuccess confirmMessage={`Delete the ${audience.label} audience? Existing knowledge using it will block deletion.`}/>
        </div>)}
      </div>
    </section>
  </>;
