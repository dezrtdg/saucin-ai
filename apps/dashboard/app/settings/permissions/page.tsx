...
type Params=Promise<{role?:string;saved?:string;error?:string}>;
...
export default async function PermissionsPage({searchParams}:{searchParams:Params}){
  ...
  return <>
    ...
    {params.saved==='1'?<div className="alert success">Role permissions saved. Changes apply to new requests immediately; users do not need to sign out first.</div>:null}
    {params.error?<div className="alert error permissionError"><strong>Permissions were not changed.</strong><span>{params.error}</span></div>:null}
    ...
    <div className="permissionsLayout">
      ...
      <section className="permissionMatrixPanel">
        {selected?<form action={saveRolePermissionsAction.bind(null,selected.id)}>
          ...
          <div className="permissionSaveBar"><div><strong>Effective access updates immediately</strong><span>Removing Dashboard access prevents this role from signing in unless the member has another allowed role.</span></div><button className="button primary largeButton" type="submit">Save role permissions</button></div>
        </form>:<div className="emptyLibrary"><strong>Select a Discord role.</strong><span>Choose a role on the left to configure its dashboard access.</span></div>}
      </section>
    </div>
  </>;
}
