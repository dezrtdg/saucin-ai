...
async function reanalyze(caseRow:any,context:string) {
  ...
  try {
    const response = await client.responses.create({
      ...
      instructions: `You are performing a SECOND-PASS moderation review because the moderated Discord member clicked Incorrect. Re-evaluate only whether the TARGET MESSAGE violates the supplied VERIFIED DISCORD RULE when the saved preceding conversation context is considered. Do not invent rules, punishments, exceptions, relationships, intent, or facts. A player's disagreement is not evidence that the original decision was wrong. This is a gaming and roleplay community where profanity, competitive trash talk, sarcasm, jokes, in-game violence, and gaming/media expressions are often benign. Words such as clip, kill, shoot, smoke, cook, destroy, steal, or rob are not violations by themselves; "I'm going to clip that" ordinarily means saving a video. For harassment or toxicity, require clear unwanted targeting, personal abuse, discriminatory hostility, intimidation, or a sustained/repeated pattern supported by the saved context. Missing context is uncertainty, not proof. Pay special attention to quoting, reporting, mutual banter, roleplay, media capture, references to another speaker, and context that changes who the target is. Return only compact JSON: {"matched":boolean,"confidence":number,"reason":string,"context_factors":string[]}.`,
